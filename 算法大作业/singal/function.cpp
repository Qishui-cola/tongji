...
//不加速求和 Kahan
float Sum(const float data[], int len) {
	float result = 0.0f;         // 最终累加结果
	float correction = 0.0f;    // Kahan 修正项
	...
	for (size_t i = 0; i < len; i++) {
		...
		// Kahan 求和公式
		float corrected_value = value - correction;
		float new_result = result + corrected_value;
		correction = (new_result - result) - corrected_value; // 更新修正项
		result = new_result;
	}
...
//不加速求最大
float Max(const float data[], int len)   //原始求最大值函数
{
	float max_data = log(sqrt(data[0]));
	for (size_t i = 0; i < len; i++)
		if (max_data < log(sqrt(data[i])))
			max_data = log(sqrt(data[i]));
